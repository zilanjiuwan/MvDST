...
parser.add_argument('--dims', type=int, default=[50], help='Number of units in hidden layer 1.')
...
for args.dataset in ['scData']:
    ...
    file.close()
    logger = get_logger()
    logger.info('Dataset:' + str('151507'))
    ...
    for seed in range(2):
        setup_seed(seed)
        print(type(sm_fea_s))
        best_accs, best_nmis, best_aris, best_f1s, prediect_labels = clustering(sm_fea_s, true_labels, args.cluster_num)
        print(best_ari)
        ...
        for epoch in tqdm(range(args.epochs)):
            ...
            S = z1 @ z2.T
            loss = 0.0001 * F.mse_loss(S, target) + criterion(decoder_out, inx) #+ criterion(decoder_out2, inx)
            loss.backward()
            optimizer.step()
            if epoch % 10 == 0:
                model.eval()
