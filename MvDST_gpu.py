...
def MvDST(features, true_labels, adj, adj_m, device='cuda:0', gnnlayers=6, epochs=400, dims=[500], lr=1e-3, sigma=0.01, cluster_num=7):
    """
    ...
    target_m = torch.FloatTensor(adj_m_1st).to(device)
    ...
    for epoch in tqdm(range(epochs)):
        ...
        S = z1 @ z2.T
        loss = 0.001 * (F.mse_loss(S, target) + 0.001* F.mse_loss(S, target_m)) + criterion(decoder_out, inx)
        ...
        if epoch % 10 == 0:
            ...
            with torch.no_grad():
                ...
                if ari > best_ari:
                    ...
                    df_X = pd.DataFrame(decoder_out.cpu().detach().numpy())
    ...
    return df_X, best_labels, best_h
